...
const ProductCart = ({ cartList, styles, openModal, totalPrice, removeCart, amountDecrement, amountIncrement, allRemoveCart, paymentCart }) => {
  ...
  const handlePayment = async (method) => {
    Swal.confirm(`결제하시겠습니까?`, `[${method}]으로 결제 합니다.`, 'question', async () => await paymentCart(method))
  }
    ...
    <>
      <div className={styles['right-side']}>
        ...
        <div className={styles['payment-buttons']}>
          <button className={styles['card-pay']} onClick={() => handlePayment('카드')}>
            <span className='material-symbols-outlined'>credit_card</span>
            <p>카드 결제</p>
          </button>
          <button className={styles['cash-pay']} onClick={() => handlePayment('현금')}>
